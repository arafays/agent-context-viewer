...
export function sessionSearchLines(
  ...
  for (const turn of turns) {
    ...
    for (const e of turn.events) {
      ...
      const body = typeof (e as { body?: string }).body === "string"
        ? (e as { body?: string }).body as string
        : detail;
    ...
    for (const call of turn.assistantCalls) {
      ...
      // thinking blocks
      const thinking = call.blocks.filter((b) => b.kind === "thinking" || b.kind === "reasoning")
        .map((b) => b.text ?? "").join("\n");
      if (thinking.trim()) {
        push(header("thinking"), collapse(thinking));
      }
      // assistant text
      const text = call.blocks.filter((b) => b.kind === "text").map((b) => b.text ?? "").join("\n");
      if (text.trim()) {
