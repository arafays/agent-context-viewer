...
export function discoverSessions(): SessionMeta[] {
  ...
  for (const r of d
    .query(`SELECT session_id, COUNT(*) c FROM part WHERE json_extract(data,'$.type')='tool' GROUP BY session_id`)
    .all() as Array<{ session_id: string; c: number }>) {
    toolCounts.set(r.session_id, num(r.c));
  }
  const compCounts = new Map<string, number>();
  for (const r of d
    .query(`SELECT session_id, COUNT(*) c FROM part WHERE json_extract(data,'$.type')='compaction' GROUP BY session_id`)
    .all() as Array<{ session_id: string; c: number }>) {
  ...
  for (const r of rows) {
    ...
    const updated = num(r.time_updated) || created;
...
function outputPreview(out: unknown): string {
...
export function loadSession(meta: SessionMeta): AgentSession {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (m.role === "user") {
      ...
      const text = textParts.map((p) => outputPreview((p as { text?: string }).text)).join("\n").trim();
    ...
    const reply = textParts.map((p) => outputPreview((p as { text?: string }).text)).join("\n").trim();
    ...
    for (const tp of toolParts) {
      ...
      const toolNorm: NormalizedMessage = {
        role: "toolResult",
        timestamp: ts,
        blocks: [{ kind: "tool_result", text: outputPreview(tp.state?.output) }],
        toolName,
