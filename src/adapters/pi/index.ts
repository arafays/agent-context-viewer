...
export function discoverSessions(cache?: MetaCache): SessionMeta[] {
  ...
  for (const slug of readdirSync(sessionsDir)) {
    ...
    for (const file of readdirSync(dir)) {
      ...
      // full parse. The source's size+mtime guard against staleness.
      if (cache) {
        const key = metaCacheKey({ tool: "pi", project: decodeSlug(slug), id: header.id })
        const entry = cache.read(key)
        if (entry && cacheIsFresh(entry, fst.size, fst.mtimeMs)) {
          metas.push({ ...entry.meta!, path, updatedAt: fst.mtime.toISOString(), sizeBytes: fst.size })
          continue
        }
      }
      const meta = metaFromHeader(path, slug, header, fst)
      if (cache) {
        cache.write(metaCacheKey({ tool: "pi", project: meta.project, id: meta.id }), {
          sizeBytes: fst.size,
          mtimeMs: fst.mtimeMs,
          meta
        })
  ...
  switch (block.type) {
    ...
    case "tool_result": {
      ...
      const text = Array.isArray(content)
        ? content.map((c: { text?: string }) => (typeof c === "object" && c ? (c.text ?? "") : String(c))).join("\n")
...
export function loadSession(path: string): PiSession {
  ...
  for (const entry of indexed) {
    if (entry.type === "message") {
      ...
      const turn = ensureTurn(ts)
      const dataStr =
        typeof entry.data === "string" ? entry.data.slice(0, 120) : JSON.stringify(entry.data ?? {}).slice(0, 120)
      turn.events.push({
        kind: "custom",
        timestamp: ts,
  ...
  const meta = metaFromHeader(
    path,
    basename(dirname(path)),
    header as { id: string; timestamp: string; cwd: string },
    statSync(path)
  )
  ...
  const contextPoints = buildContextPoints(entries, assistantCalls, turns)
