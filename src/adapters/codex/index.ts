...
function readFirstLine(path: string): string {
  try {
    ...
    try {
      // Chunk-scan for the first complete newline-terminated line: the first
      // line is session_meta, which can exceed 64KB when base_instructions
      // (system prompt) is embedded — truncating it loses cwd/id.
      const chunks: Buffer[] = []
      let total = 0
      while (total < MAX_FIRST_LINE_BYTES) {
        const buf = Buffer.alloc(SCAN_BYTES)
        const n = readSync(fd, buf, 0, SCAN_BYTES, total)
        if (n <= 0) break
        total += n
        const valid = buf.subarray(0, n)
        const nl = valid.indexOf("\n")
        if (nl !== -1) {
          return Buffer.concat([...chunks, valid.subarray(0, nl + 1)]).toString("utf8")
        }
        chunks.push(valid)
      }
...
export function discoverSessions(cache?: MetaCache): SessionMeta[] {
  ...
  for (const path of files) {
    try {
      ...
      // Cache hit: reuse the parsed meta (counters + searchText) without
      // reading the whole file twice.
      if (cache) {
        const key = metaCacheKey({ tool: "codex", project, id })
      ...
      if (cache) {
        cache.write(metaCacheKey({ tool: "codex", project, id }), {
          sizeBytes: st.size,
          mtimeMs: st.mtimeMs,
          meta
        })
...
export function loadSession(path: string): AgentSession {
      ...
      case "event_msg": {
        ...
        if (kind === "task_started") {
          ...
          const info = p.info as
            | { last_token_usage?: Record<string, number>; total_token_usage?: Record<string, number> }
            | undefined
  ...
  for (let ti = 0; ti < perTurn.length; ti++) {
    ...
    const realPrompt =
      [...userItems]
        .reverse()
        .find(
          (g) => !g.text?.startsWith("# AGENTS.md instructions for") && !g.text?.trimStart().startsWith("<skill>")
    ...
    for (const item of group) {
      ...
      if (item.kind === "developer" && item.text?.startsWith("<permissions instructions>")) {
        turn.events.push({
          kind: "custom",
          timestamp: item.timestamp,
          detail: "permissions instructions loaded (inline)"
        })
      }
    }
    turns.push(turn)
  ...
  for (const req of requests) {
    ...
    // within this request's span — never the next request's output
    const firstOutput = allItemsByLine.find(
      (it) => it.line > prevRequestLine && it.line <= req.line && (it.kind === "reasoning" || it.kind === "assistant")
    )
    if (!firstOutput) {
    ...
    const usage: UsageTotals = {
      ...req.usage,
      total: req.usage.input + req.usage.output + req.usage.cacheRead + req.usage.cacheWrite
    }
