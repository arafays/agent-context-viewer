...
type CBlock = { type?: string; text?: string; thinking?: string; name?: string; id?: string; input?: unknown; content?: unknown; is_error?: boolean };
...
export function loadSession(path: string): AgentSession {
  ...
  const rawAssistants: Array<{ o: RawLine; lineNo: number; ts: number }> = [];
  ...
  const normMessage = (o: RawLine, role: NormalizedMessage["role"], entryIndex: number, extra?: Partial<NormalizedMessage>): NormalizedMessage => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    switch (o.type) {
      ...
      case "assistant": {
        ...
        rawAssistants.push({ o, lineNo, ts });
        turn.entryEnd = lineNo;
        break;
      }
      case "attachment": {
  ...
  for (const line of lines) {
    ...
    if (o?.timestamp && !startedAt) startedAt = o.timestamp;
  ...
  for (let k = 0; k < assistantCalls.length; k++) {
    ...
    const before = ordered.filter((x) => x.line < call.entryIndex).map((x) => x.msg);
  ...
  const contextInfo: SessionContextInfo = {
    ...
    skills: [],
    tools: [...new Set(assistantCalls.flatMap((c) => c.blocks.filter((b) => b.kind === "tool_use").map((b) => b.toolName ?? "")))]
      .filter(Boolean)
      .sort(),
    reconstructed: true,
    notes: [
